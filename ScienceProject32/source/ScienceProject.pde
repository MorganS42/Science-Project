...
void na(Boolean w, String s) { //news add
  for(news n : News) {
    n.move(true);  
  }
  ...
  if(!p) {
    if(!bc) {
      ...
      for(int i=0; i<Pile.length; i++) {
        if(Pile[i].trc>=round(Pile[i].max/2.0)*round(Pile[i].max/2.0)) {
          if(!warns[i]) {
            warns[i]=true;
            if(i==0) {
      ...
      if(Pile[0].trc>=round(Pile[0].max/2.0)*round(Pile[0].max/2.0)) {
        supply=0;  
      }
      time++;
      if(time>speed/2) {
      ...
      if(time==0) {
        ...
        if(supply>demand) {
          ...
          hap=supply/demand*100.0;
        }
        if(supply>demand) {
          bud=round(hap*demand/10-2500.0);
        }
        else {
          bud=round(hap*supply/10-2500.0);  
        }
        if(ec>tc) {
          bud-=tc*1000;
        }
      ...
      //println(time);
      fill((t/(speed))*130.0,(t/(speed))*200.0,(t/(speed))*255.0);
      stroke((t/(speed))*130.0,(t/(speed))*200.0,(t/(speed))*255.0);
      ...
      translate(120, 200); 
      rotate(map(time%(speed/2.0),0.0,speed/2.0,-PI,PI));//random(-PI,PI));
      rect(-5,-5,10,60);
      
      rotate(-map(time%(speed/2.0),0.0,speed/2.0,-PI,PI));
      translate(-120,-200);
...
class pile {
  ...
  void us() { //up size
    ...
    for(int i=0; i<size; i++) {
      pos.add(new PVector(x+(i-floor(size/2.0))*ts,y+(abs(i-floor(size/2.0))-floor(size/2.0))*ts));  
      boo.add(false);
    }
  }
  ...
  void run() {
    ...
    switch(type) {
      case 0:
        ...
        text(tc,x+w/2.2,y+h/6);
        image(truck,x+w/3.7,y+h/10,h/10,h/12);
        strokeWeight(6);
        stroke(0);
        ...
        yo=0;
        fill(200);
        if(mouseX>x+w/3.5+xo && mouseX<x+w/3.5+xo+h/12 && mouseY>yo+y+h/10 && mouseY<yo+y+h/10+h/12) { 
          textbox(x+w/3.5+xo+h/24,yo+y+h/10+h/24,400,200,600,300,"Selling a truck will give you half of the original price back, $5,000. "+(tc == 0 ? "You don't have any trucks so you can't sell any!" : ""));
          fill(150);  
          if(mousec && tc>0) {
            money+=5000;
            tc--;
        ...
        rect(x+w/3.5+xo,y+h/10+yo,h/12,h/12);
        line(x+w/3.5+10+xo,y+h/10+h/24+yo,x+w/3.5+h/12-10+xo,y+h/10+h/24+yo);
        
        fill(200);
        if(mouseX>x+w/3.5+xo+h/12 && mouseX<x+w/3.5+xo+h/12+h/12 && mouseY>y+h/10+yo && mouseY<y+h/10+h/12+yo) {
          textbox(x+w/3.5+xo+h/24*3,y+h/10+h/24,400-h/12+yo,200,600,300,"A truck costs $10,000. You need one employe to run one truck, a truck picks up 1000 pieces of garbage a day."+(money < 10000 ? "You don't have enough money!" : ""));
          fill(150);
        ...
        rect(x+w/3.5+h/12+xo,y+h/10+yo,h/12,h/12);
        line(x+w/3.5+10+h/12+xo,yo+y+h/10+h/24,x+w/3.5+h/12-10+h/12+xo,yo+y+h/10+h/24);
        line(x+w/3.5+h/24+h/12+xo,yo+y+h/10+10,x+w/3.5+h/12+h/24+xo,yo+y+h/10+h/12-10);
        ...
        text(ec,x+w/2.2,y+h/6+100);
        image(worker,x+w/3.7,y+h/10+100,h/8,h/12);
        strokeWeight(6);
        stroke(0);
        ...
        fill(200);
        if(mouseX>x+w/3.5+xo && mouseX<x+w/3.5+xo+h/12 && mouseY>yo+y+h/10 && mouseY<yo+y+h/10+h/12) { 
          textbox(x+w/3.5+xo+h/24,yo+y+h/10+h/24,400,200,600,300,"Selling an employee will save you $1,000 a week."+(ec == 0 ? "You don't have any employees so you can't sell any!" : ""));
          fill(150);  
          if(mousec && ec>0) {
            ec--;
          }
        ...
        fill(200);
        if(mouseX>x+w/3.5+xo+h/12 && mouseX<x+w/3.5+xo+h/12+h/12 && mouseY>y+h/10+yo && mouseY<y+h/10+h/12+yo) {
          textbox(x+w/3.5+xo+h/24*3,y+h/10+h/24+yo,400-h/12,200,700,300,"An employee costs $200 and a salary $1,000 a week. Each employee runs one truck. Employees without a truck available will not be payed. "+(money < 100 ? "You don't have enough money!" : ""));
          fill(150);
          if(mousec && money>=200) {
            ec++;
            money-=200;
      ...
      case 1:
        ...
        text(sc1,x+w/2.2,y+h/6);
        image(worker,x+w/3.7,y+h/10,h/8,h/12);
        strokeWeight(6);
        stroke(0);
        ...
        fill(200);
        if(mouseX>x+w/3.5+xo && mouseX<x+w/3.5+xo+h/12 && mouseY>yo+y+h/10 && mouseY<yo+y+h/10+h/12) { 
          textbox(x+w/3.5+xo+h/24,yo+y+h/10+h/24,400,200,600,300,"Selling a worker stops you from paying $800 a week."+(sc1 == 0 ? "You don't have any workers so you can't sell any!" : ""));
          fill(150);  
          if(mousec && sc1>0) {
            sc1--;
          }
        ...
        fill(200);
        if(mouseX>x+w/3.5+xo+h/12 && mouseX<x+w/3.5+xo+h/12+h/12 && mouseY>y+h/10+yo && mouseY<y+h/10+h/12+yo) {
          textbox(x+w/3.5+xo+h/24*3,y+h/10+h/24,400-h/12+yo,200,600,300,"A worker costs $150 and has a salary of $800 per week. A human worker sorts 200 items in a day."+(money < 150 ? "You don't have enough money!" : ""));
          fill(150);
          if(mousec && money>=150) {
            money-=150;
            sc1++;
        ...
        text(sc2,x+w/2.2,y+h/6+100);
        image(s1,x+w/3.5,y+h/10+100,h/13,h/13);
        strokeWeight(6);
        stroke(0);
        ...
        fill(200);
        if(mouseX>x+w/3.5+xo && mouseX<x+w/3.5+xo+h/12 && mouseY>yo+y+h/10 && mouseY<yo+y+h/10+h/12) { 
          textbox(x+w/3.5+xo+h/24,yo+y+h/10+h/24,400,200,600,300,"Selling a Basic Sorter will give you half the original price, $4,000 and will stop you from paying the weekly fee of $500."+(sc2 == 0 ? "You don't have any Basic Sorters so you can't sell any!" : ""));
          fill(150);  
          if(mousec && sc2>0) {
            sc2--;
          }
        ...
        fill(200);
        if(mouseX>x+w/3.5+xo+h/12 && mouseX<x+w/3.5+xo+h/12+h/12 && mouseY>y+h/10+yo && mouseY<y+h/10+h/12+yo) {
          textbox(x+w/3.5+xo+h/24*3,y+h/10+h/24+yo,400-h/12,200,700,300,"A Basic Sorter costs $8,000 and $100 a week to maintain. A basic sorter sorts 1000 items a day."+(money < 8000 ? "You don't have enough money!" : ""));
          fill(150);
        ...
        text(sc3,x+w/2.2,y+h/6+200);
        image(s2,x+w/3.5,y+h/10+200,h/13,h/13);
        strokeWeight(6);
        stroke(0);
        ...
        fill(200);
        if(mouseX>x+w/3.5+xo && mouseX<x+w/3.5+xo+h/12 && mouseY>yo+y+h/10 && mouseY<yo+y+h/10+h/12) { 
          textbox(x+w/3.5+xo+h/24,yo+y+h/10+h/24,400,200,600,300,"Selling a Advanced Sorter will give you half the original price, $10,000 and will stop you from paying the weekly fee of $200."+(ec == 0 ? "You don't have any Advanced Sorters so you can't sell any!" : ""));
          fill(150);  
          if(mousec && sc3>0) {
            sc3--;
          }
        ...
        fill(200);
        if(mouseX>x+w/3.5+xo+h/12 && mouseX<x+w/3.5+xo+h/12+h/12 && mouseY>y+h/10+yo && mouseY<y+h/10+h/12+yo) {
          textbox(x+w/3.5+xo+h/24*3,y+h/10+h/24+yo,400-h/12,200,700,300,"A Advanced Sorter costs $20,000 and $100 a week to maintain. A Advanced Sorter sorts 10000 items a day."+(money < 20000 ? "You don't have enough money!" : ""));
          fill(150);
...
void textbox(float x,float y,float xt, float yt, float w,float h, String t) {
  strokeWeight(8);
  stroke(80);
